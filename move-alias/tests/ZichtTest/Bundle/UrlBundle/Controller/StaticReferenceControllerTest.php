...
class StaticReferenceControllerTest extends \PHPUnit_Framework_TestCase
{
    public function testRedirectAction()
    {
        ...
        $provider = $this->getMock('Zicht\Bundle\UrlBundle\Url\SuggestableProvider');
        $container->set('zicht_url.provider', $provider);
        $controller->setContainer($container);

        $provider->expects($this->once())->method('suggest')->with('foo')->will($this->returnValue(
            array(
                array('a' => 'b')
            )
        ));
